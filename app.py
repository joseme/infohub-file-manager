...
def main(page: ft.Page):
    try:
        ...
        traceback.print_exc()
        page.add(ft.Column([
            ft.Text(f"Error: {e}", color=ft.Colors.RED_500, size=16),
        ]))
        page.update()
...
def _main(page: ft.Page):
    ...
    def build_config_editor():
        ...
        for field in config_schema:
            ...
            if field["type"] == "bool":
                ...
                row = ft.Row(
                    [
                        ft.Text(label, size=13, color=Colors.GREY_700),
                        ft.Container(expand=True),
                        sw,
                    ],
                ...
                row = ft.Row(
                    [
                        ft.Text(label, size=13, color=Colors.GREY_700, expand=True),
                        dd,
                    ],
                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
                )
                ...
                tf = ft.TextField(
                    ...
                    focused_border_color=Colors.BLUE_500,
                    width=280,
                )
    ...
    config_dialog = ft.AlertDialog(
        ...
        actions=[
            ft.TextButton(
                "Cancelar",
                icon=Icons.CLOSE,
                on_click=lambda e: setattr(config_dialog, "open", False) or page.update(),
            ),
            ft.FilledButton(
                "Guardar",
                icon=Icons.SAVE,
    ...
    app_bar = ft.AppBar(
        ...
        title=ft.Column(
            [
                ...
                ft.Text("Sincronizacion con InfoHub", size=11, color=Colors.BLUE_100),
            ],
            spacing=0,
        ),
        center_title=False,
    ...
    # Config Panel
    config_api_text = ft.Text(manager.base_url, size=13, color=Colors.GREY_800)
    config_ollama_text = ft.Text(f"{manager.ollama_url} ({manager.ollama_model})", size=13, color=Colors.GREY_800)
    config_root_text = ft.Text(manager.watched_root or "No configurado", size=13, color=Colors.GREY_800)
    config_img_desc_text = ft.Text(
    ...
    def update_config_panel():
        config_api_text.value = manager.base_url
        config_ollama_text.value = f"{manager.ollama_url} ({manager.ollama_model})"
        config_root_text.value = manager.watched_root or "No configurado"
        config_img_desc_text.value = "Habilitado" if manager.image_description_active else "Deshabilitado"
        config_img_desc_text.color = Colors.GREEN_700 if manager.image_description_active else Colors.RED_700
        page.update()
